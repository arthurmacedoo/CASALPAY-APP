 ...
 * Na Etapa 2.1, não dependemos do role "admin" para o Partner porque a migração
 * colocou ambos como admin. Usamos o nome de forma robusta.
 */
export function getLegacyRoleForMember(member: GroupMember | null): "owner" | "partner" | null {
  if (!member) return null;
  const name = member.name.toLowerCase();
  if (name.includes(OWNER_NAME.toLowerCase())) return "owner";
  if (name.includes(PARTNER_NAME.toLowerCase())) return "partner";
  
  // Fallback seguro caso os nomes mudem e não dê match (mantendo o legado de fallback)
  if (member.role === "admin" && !name.includes(PARTNER_NAME.toLowerCase())) return "owner";
