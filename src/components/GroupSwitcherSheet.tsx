...
export const GroupSwitcherSheet: React.FC<GroupSwitcherSheetProps> = ({
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-50 flex flex-col bg-bg-card animate-slide-up"
    >
      {/* Header respeitando a safe area (bateria/relógio) */}
      <div className="pt-[calc(env(safe-area-inset-top)+16px)] pb-4 px-5 flex items-center justify-between border-b border-border bg-bg-card shrink-0">
        <h2 className="text-[28px] font-bold text-text-primary tracking-tight">Meus Grupos</h2>
        <button 
          onClick={onClose}
          className="w-10 h-10 rounded-full bg-bg-elevated flex items-center justify-center text-text-muted hover:text-text-primary hover:bg-white/5 transition-colors"
        >
      ...
      <div
        ref={sheetRef}
        className="flex-1 overflow-y-auto px-5 pt-6 pb-20 safe-bottom"
      >
        {/* Título de seção */}
